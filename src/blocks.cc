...
// ================================= ParticleBlock =====================================
ParticleBlock ParticleBlock::read_from(std::ifstream& bfile, size_t psize, const Format& /*fmt*/) {
    ParticleBlock p;
    ...
    const size_t bytes = checked_mul(static_cast<size_t>(p.npart), psize, "ParticleBlock bytes");
    p.particles = read_chunk(bfile, bytes);
    return p;
}

// =============================== InteractionBlock ====================================
InteractionBlock InteractionBlock::read_from(std::ifstream& bfile, size_t psize, const Format& fmt) {
    InteractionBlock ib;
    ib.particle_size = psize;

    // header bytes in one go (adjust size if fields change with fmt)
    constexpr size_t HDR = sizeof(int32_t) + sizeof(int32_t) + 3*sizeof(double) + sizeof(int32_t);
    auto header = read_chunk(bfile, HDR);
    size_t off = 0;
    ...
    const size_t nin_bytes  = checked_mul(ib.n_in_sz(),  psize, "Interaction incoming");
    const size_t nout_bytes = checked_mul(ib.n_out_sz(), psize, "Interaction outgoing");
