    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for col in (9, 10, 11):
            assert np.array_equal(
                rp[:, col].astype(int), wp[:, col].astype(int)
...
def test_read_interaction_block():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with open(path, "wb") as f:
            writeHeader(f)
            writeParticleBlock(
                f, event_number=0, ensemble_number=0, particles=[]
            )  # optional
            incoming = generateParticles(2)
            outgoing = generateParticles(3)
            writeInteractionBlock(
                f, incoming, outgoing, rho=0.25, sigma=7.5, sigma_p=0.4, process=17
            )
        ...
        for k in fields_d:
            ...
            assert np.allclose(
                acc.get_incoming_array(k), exp_in, rtol=1e-12, atol=1e-12
            )
            assert np.allclose(
                acc.get_outgoing_array(k), exp_out, rtol=1e-12, atol=1e-12
            )
