...
static py::array make_block_array(
    ...
    if (N == 0) {
        if (qtype == QuantityType::Double) return py::array_t<double>(0);
        else                               return py::array_t<int32_t>(0);
    }
...
m.def("register_python_analysis",
      [](const std::string& name,
         py::object py_factory,
         py::dict opts) {
          AnalysisRegistry::instance().register_factory(
              name,
              [name, py_factory, opts]() -> std::shared_ptr<Analysis> {
                  pybind11::gil_scoped_acquire gil;
      ...
      py::arg("name"),
      py::arg("factory"),
      py::arg("opts") = py::dict{}   // default empty dict, also enables keyword
);
...
py::class_<Accessor, PyAccessor, std::shared_ptr<Accessor>>(m, "Accessor")
    ...
    .def("get_block_double_array",
         [](const Accessor& self, const ParticleBlock& block, const std::string& name) {
             ...
             return make_block_array(block, self.layout_map(), name, QuantityType::Double);
         })
    .def("get_block_int_array",
         [](const Accessor& self, const ParticleBlock& block, const std::string& name) {
             ...
             return make_block_array(block, self.layout_map(), name, QuantityType::Int32);
         })
...
py::class_<BinaryReader>(m, "BinaryReader")
    ...
    .def("read", &BinaryReader::read, py::call_guard<py::gil_scoped_release>());

// --- CollectorAccessor: return NumPy views with a base to keep memory alive ---
