...
class Dndydmt:
    ...
    def on_particle_block(self, block, accessor, opts):
        ...
        # tracked pdgs
        if self.track:
            present_tracked = np.intersect1d(np.unique(pdg), np.fromiter(self.track, dtype=int))
            for val in present_tracked:
                sel = (pdg == val)
                H = self.per_pdg.setdefault(int(val), HistND([self.mt_edges, self.y_edges]))
...
br.register_python_analysis(
    "dndydmt",
    lambda: Dndydmt(edges_y, edges_mt),
    {}
)
